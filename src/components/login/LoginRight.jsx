...
const LoginRight = () => {
  ...
  return (
    <motion.div
      ...
      className="w-full p-[50px] bg-white "
    >
      <div className="text-center">
        <h2 className=" text-[48px] font-semibold text-[#262626] ">Login</h2>
      ...
      <div className="mt-[50px]">
        ...
        <div className="w-full bg-[#FCFCFD] border-[1px] mt-[14px] border-[solid]  border-[#F1F1F3] rounded-[10px] p-6 flex items-center justify-between">
          <input type={`${eye ? "text" : "password"}`} className=" outline-0" />
          {eye ? (
        ...
        <Link
          to={"/signUp"}
          className=" w-full flex items-center justify-center mt-[30px] cursor-pointer hover:text-[#ff9500]"
        >
          Don’t have an account? Sign Up <RxArrowTopRight />
        </Link>
      </div>
    </motion.div>
  );
